...
void V114ShaderMultiplexFileTraceRaw(const char* message) {
    ...
    std::FILE* fp = std::fopen("/tmp/borked3ds_v114c11_shader_probe.log", "a");
    if (fp == nullptr) {
        return;
    }
...
void V114ShaderMultiplexFileTraceReset() {
    ...
    std::FILE* fp = std::fopen("/tmp/borked3ds_v114c11_shader_probe.log", "w");
    if (fp == nullptr) {
        return;
    }
    std::fputs("v114c11 file_trace_reset\n", fp);
    std::fclose(fp);
}
...
RasterizerVulkan::RasterizerVulkan(Memory::MemorySystem& memory, Pica::PicaCore& pica,
    ...
    if (IsV114ShaderMultiplexFileTraceEnabled()) {
        ...
        V114ShaderMultiplexFileTraceRaw("v114c11 rasterizer_constructor_prepredraw_marker");
        V114ShaderMultiplexFileTraceNumber("v114c11 constructor_generate_guarded_probe",
                                           static_cast<u64>(IsProgrammableVertexShaderGenerateGuardedProbeEnabled()));
        V114ShaderMultiplexFileTraceNumber("v114c11 constructor_stage_stop_after",
                                           GetAccelStageStopAfter());
    }

    if (IsDrawTraceEnabled()) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_DRAW strict_compat v114c11 RasterizerVulkan constructor quietpresent-direct-accel marker strict_compat={} allow_software_textures={} quarantine_disabled={}",
...
bool RasterizerVulkan::SetupVertexShader() {
    ...
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 setup_vs_begin");
        if (programmable_config_probe) {
            V114ShaderMultiplexFileTraceRaw("v114c11 setup_vs_probe=config_only");
        }
        if (programmable_before_generate_probe) {
            V114ShaderMultiplexFileTraceRaw("v114c11 setup_vs_probe=before_generate_only");
        }
        if (programmable_generate_guarded_probe) {
            V114ShaderMultiplexFileTraceRaw("v114c11 setup_vs_probe=generate_guarded_only");
        }
        if (programmable_spirv_probe) {
            V114ShaderMultiplexFileTraceRaw("v114c11 setup_vs_probe=spirv_only");
        }
        if (programmable_module_probe) {
            V114ShaderMultiplexFileTraceRaw("v114c11 setup_vs_probe=shader_module_only");
        }
        V114ShaderMultiplexFileTraceNumber("v114c11 setup_vs_binding_count",
                                           pipeline_info.vertex_layout.binding_count);
        V114ShaderMultiplexFileTraceNumber("v114c11 setup_vs_attribute_count",
    ...
    if (programmable_before_generate_probe) {
        ...
        if (v114_file_trace) {
            V114ShaderMultiplexFileTraceRaw("v114c11 shader_probe_config_built");
            V114ShaderMultiplexFileTraceNumber("v114c11 shader_probe_config_hash", config.Hash());
            V114ShaderMultiplexFileTraceNumber("v114c11 shader_probe_converted_attribs",
                                               converted_attribs);
            V114ShaderMultiplexFileTraceNumber("v114c11 shader_probe_zero_w_attribs",
                                               zero_w_attribs);
        }
    ...
    auto run_shader_multiplex_probe = [&](const char* mode_name, bool compile_spirv,
        ...
        if (v114_file_trace) {
            V114ShaderMultiplexFileTraceRaw("v114c11 shader_probe_begin");
        ...
        try {
            if (v114_file_trace) {
                V114ShaderMultiplexFileTraceRaw("v114c11 shader_probe_before_generate_call");
            }
            if (trace_accel) {
                LOG_WARNING(Render_Vulkan,
            ...
            if (v114_file_trace) {
                V114ShaderMultiplexFileTraceRaw("v114c11 shader_probe_after_generate_call");
            }
            if (trace_accel) {
                LOG_WARNING(Render_Vulkan,
        ...
        if (v114_file_trace) {
            V114ShaderMultiplexFileTraceRaw("v114c11 shader_probe_glsl_end");
            V114ShaderMultiplexFileTraceNumber("v114c11 shader_probe_program_bytes", program.size());
            V114ShaderMultiplexFileTraceNumber("v114c11 shader_probe_program_empty",
    ...
    if (programmable_generate_guarded_probe) {
        if (v114_file_trace) {
            V114ShaderMultiplexFileTraceRaw("v114c11 shader_probe_generate_guarded_selected");
        }
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
...
bool RasterizerVulkan::AccelerateDrawBatch(bool is_indexed) {
    ...
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 accel_enter");
        V114ShaderMultiplexFileTraceNumber("v114c11 accel_id", accel_id);
        V114ShaderMultiplexFileTraceNumber("v114c11 accel_indexed", static_cast<u64>(is_indexed));
        V114ShaderMultiplexFileTraceNumber("v114c11 accel_num_vertices",
                                           regs.pipeline.num_vertices);
        V114ShaderMultiplexFileTraceNumber("v114c11 accel_topology",
    ...
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 before_analyze_vertex_array");
    }
    vertex_info = AnalyzeVertexArray(is_indexed, instance.GetMinVertexStrideAlignment());
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 after_analyze_vertex_array");
    }
    ...
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 before_setup_vertex_array");
    }
    SetupVertexArray();
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 after_setup_vertex_array");
    }
    ...
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 before_setup_vertex_shader");
    }
    if (!SetupVertexShader()) {
        if (v114_file_trace) {
            V114ShaderMultiplexFileTraceRaw("v114c11 setup_vertex_shader_result=0");
        }
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
    ...
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 setup_vertex_shader_result=1");
    }
    ...
    if (v114_file_trace) {
        V114ShaderMultiplexFileTraceRaw("v114c11 before_stage7_consume");
    }
    if (consume_if_stage_limited(7, stage7_name)) {
        if (v114_file_trace) {
            V114ShaderMultiplexFileTraceRaw("v114c11 stage7_consumed_return_true");
        }
        return true;
    }
