...
[[nodiscard]] bool IsTrivialVertexShaderProbeEnabled() {
    ...
    // path is stable. Keep this switch as an explicit fallback, but v99 should normally
    // leave it disabled.
...
[[nodiscard]] bool IsProgrammableVertexShaderGenerateProbeEnabled() {
    ...
    // but v99 should normally leave it disabled.
...
[[nodiscard]] bool IsStrictAccelInternalDryRunEnabled() {
    // v99: plan de travail 1 does not advance beyond stage=7. v98 proved the programmable
    // PICA VS config/load_flags path reaches stage=7 and returns true. The recommended v99
    // emulators.cfg keeps BORKED3DS_V3DV_ACCEL_STAGE_STOP_AFTER=7 and enables
    // BORKED3DS_V3DV_PROBE_PROGRAMMABLE_VS_GENERATE_GUARDED_ONLY=1, so SetupVertexShader()
    // calls only GLSL::GenerateVertexShader() with explicit guarded logs, then binds the
    // trivial VS again. This isolates GLSL generation from SPIR-V compile, shader module
    // creation, geometry shader setup, pipeline bind, descriptors, Draw(), and vkCmdDraw.
    return IsStrictCompatEnabled() &&
...
[[nodiscard]] bool IsStrictSafeUntexturedSoftwareDrawAllowed() {
    ...
    return IsEnvEnabled("BORKED3DS_V3DV_ALLOW_SAFE_UNTEXTURED_SOFTWARE_DRAWS") &&
           IsEnvEnabled("BORKED3DS_V3DV_ALLOW_V99_REAL_SOFTWARE_DRAWS") &&
...
[[nodiscard]] bool IsAccelEntryPreflightExpected() {
    // v99 normal test: enter AccelerateDrawBatch, pass stage=1 through stage=6,
    // run SetupVertexShader() in programmable-VS GLSL-generate-guarded-only mode,
    // then stop at stage=7. This still returns before SPIR-V compile/module creation,
    // geometry shader setup completion, pipeline state build, descriptors, Draw(), or vkCmdDraw.
...
RasterizerVulkan::RasterizerVulkan(Memory::MemorySystem& memory, Pica::PicaCore& pica,
    ...
    if (IsDrawTraceEnabled()) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_DRAW strict_compat v99 RasterizerVulkan constructor marker strict_compat={} allow_software_textures={} quarantine_disabled={}",
...
bool RasterizerVulkan::SetupVertexShader() {
    ...
    if (trace_accel) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_ACCEL_STAGE v99 vertex_shader_setup_begin trivial_probe={} programmable_config_probe={} programmable_generate_guarded_probe={} programmable_generate_probe={} binding_count={} attribute_count={} accurate_mul={} strict_compat={}",
    ...
    if (trivial_vs_probe) {
        ...
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_trivial_probe_used result=1");
        }
        return true;
    }
    ...
    if (programmable_config_probe) {
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_programmable_config_only_begin");
        }
        ...
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_programmable_config_only_end use_geometry_shader={} converted_attribs={} zero_w_attribs={} config_hash={}",
        ...
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_programmable_config_only_trivial_bind result=1");
        }
    ...
    auto run_generate_only_probe = [&](const char* mode_name) -> bool {
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_{}_begin", mode_name);
        }
        ...
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_{}_config use_geometry_shader={} converted_attribs={} zero_w_attribs={} config_hash={}",
        ...
        if (programmable_generate_guarded_probe) {
            try {
                if (trace_accel) {
                    LOG_WARNING(Render_Vulkan,
                                "TRACE_ACCEL_STAGE v99 vertex_shader_setup_{}_before_generate_call",
                                mode_name);
                }
                ...
                if (trace_accel) {
                    LOG_WARNING(Render_Vulkan,
                                "TRACE_ACCEL_STAGE v99 vertex_shader_setup_{}_after_generate_call",
                                mode_name);
                }
            } catch (const std::exception& e) {
                LOG_ERROR(Render_Vulkan,
                          "TRACE_ACCEL_STAGE v99 programmable VS GLSL generation threw std::exception: {}",
                          e.what());
                return false;
            } catch (...) {
                LOG_ERROR(Render_Vulkan,
                          "TRACE_ACCEL_STAGE v99 programmable VS GLSL generation threw unknown exception");
                return false;
            }
        } else {
            program = GLSL::GenerateVertexShader(pica.vs_setup, config, true);
        ...
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_{}_end program_bytes={} empty={}",
                        mode_name, program.size(), static_cast<u32>(program.empty()));
        }

        if (program.empty()) {
            LOG_ERROR(Render_Vulkan,
                      "TRACE_ACCEL_STAGE v99 programmable VS GLSL generation returned empty program");
            return false;
        }
        ...
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_{}_trivial_bind result=1",
                        mode_name);
        }
    ...
    if (trace_accel) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_ACCEL_STAGE v99 vertex_shader_setup_programmable_begin");
    }
    ...
    if (trace_accel) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_ACCEL_STAGE v99 vertex_shader_setup_programmable_end result={}",
...
bool RasterizerVulkan::AccelerateDrawBatch(bool is_indexed) {
    ...
    const auto log_stage = [&](u32 stage, const char* name) {
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 accel_id={} stage={} name={} indexed={} num_vertices={} topology={} use_gs={} preflight_expected={} color_addr=0x{:08x} depth_addr=0x{:08x}",
    ...
    const auto consume_if_stage_limited = [&](u32 stage, const char* name) {
        ...
        if (ShouldStopAfterAccelStage(stage)) {
            if (trace_accel) {
                LOG_WARNING(Render_Vulkan,
                            "TRACE_ACCEL_STAGE v99 stage_limit consumed accel_id={} stage={} name={} stop_after={} before_vulkan_command=1",
    ...
    if (regs.pipeline.use_gs != Pica::PipelineRegs::UseGS::No) {
        if (regs.pipeline.gs_config.mode != Pica::PipelineRegs::GSMode::Point) {
            if (trace_accel) {
                LOG_WARNING(Render_Vulkan,
                            "TRACE_ACCEL_STAGE v99 rejected_gs_mode accel_id={} gs_mode={}",
        ...
        if (regs.pipeline.triangle_topology != Pica::PipelineRegs::TriangleTopology::Shader) {
            if (trace_accel) {
                LOG_WARNING(Render_Vulkan,
                            "TRACE_ACCEL_STAGE v99 rejected_gs_topology accel_id={} topology={}",
    ...
    if (regs.pipeline.triangle_topology == TriangleTopology::Fan &&
        ...
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 rejected_triangle_fan accel_id={}", accel_id);
        }
        return false;
    }
    ...
    if (!SetupVertexShader()) {
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 vertex_shader_setup_failed accel_id={} trivial_probe={} programmable_config_probe={} programmable_generate_guarded_probe={} programmable_generate_probe={}",
    ...
    if (!SetupGeometryShader()) {
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 geometry_shader_setup_failed accel_id={}", accel_id);
        }
        return false;
    }
    ...
    if (trace_accel) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_ACCEL_STAGE v99 accel_id={} stage=18 name=after_draw_wrapper result={}",
                    accel_id, result);
    }
    return result;
}
...
bool RasterizerVulkan::AccelerateDrawBatchInternal(bool is_indexed) {
    ...
    const auto log_stage = [&](u32 stage, const char* name) {
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 internal stage={} name={} indexed={} vertex_count={} binding_count={} dry_run={} stop_after={}",
    ...
    const auto consume_if_stage_limited = [&](u32 stage, const char* name) {
        ...
        if (ShouldStopAfterAccelStage(stage)) {
            if (trace_accel) {
                LOG_WARNING(Render_Vulkan,
                            "TRACE_ACCEL_STAGE v99 internal stage_limit consumed stage={} name={} before_vulkan_command=1",
                            stage, name);
            }
            return true;
        }
    ...
    if (regs.pipeline.num_vertices == 0) {
        if (trace_accel) {
            LOG_INFO(Render_Vulkan, "TRACE_ACCEL_STAGE v99 internal skipped empty draw");
        }
        return true;
    }
    ...
    if (binding_count == 0 || binding_count > vertex_buffers.size()) {
        ...
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 internal invalid_binding_count binding_count={} max={}",
                        binding_count, vertex_buffers.size());
        }
        return false;
    }
    ...
    if (!pipeline_cache.BindPipeline(pipeline_info, wait_built)) {
        if (trace_accel) {
            LOG_INFO(Render_Vulkan,
                     "TRACE_ACCEL_STAGE v99 pipeline_not_ready wait_built={} strict_compat={}",
    ...
    if (IsStrictAccelInternalDryRunEnabled()) {
        if (trace_accel) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_ACCEL_STAGE v99 internal dry-run consumed before vkCmdDraw stage=16 indexed={} vertex_count={} vertex_offset={} binding_count={} wait_built={}",
    ...
    if (trace_accel) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_ACCEL_STAGE v99 internal stage=17 name=vkcmd_recorded indexed={} vertex_count={} binding_count={}",
...
bool RasterizerVulkan::Draw(bool accelerate, bool is_indexed) {
    ...
    if (strict_safe_untextured_real_draw && IsDrawTraceEnabled()) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_DRAW strict_compat v99 allowing safe untextured real software draw safe_index={} budget={} vertex_batch_size={} num_vertices={} color_addr=0x{:08x} depth_addr=0x{:08x}",
    ...
    if (!accelerate && IsStrictCompatEnabled() && !IsStrictSoftwareNoopGuardDisabled() &&
        ...
        if (IsDrawTraceEnabled()) {
            LOG_WARNING(Render_Vulkan,
                        "TRACE_DRAW strict_compat v99 software fallback consumed as safe no-op vertex_batch_size={} num_vertices={} enabled_textures={} textures_disabled={} depth_active={} color_addr=0x{:08x} depth_addr=0x{:08x}; allow_safe_untextured={} safe_candidate={} set BORKED3DS_V3DV_ALLOW_REAL_SOFTWARE_DRAWS=1 only for full diagnosis",
    ...
    if (!accelerate) {
        ...
        if (IsStrictCompatEnabled() && !IsSoftwareSkipAllowed() && IsDrawTraceEnabled()) {
            LOG_INFO(Render_Vulkan,
                     "TRACE_DRAW strict_compat v99 software skip disabled; drawing software batch vertex_batch_size={} num_vertices={} enabled_textures={} textures_disabled={} depth_active={} color_addr=0x{:08x} depth_addr=0x{:08x}",
                     vertex_batch.size(), regs.pipeline.num_vertices,
    ...
    if (strict_software_null_texture_path && IsDrawTraceEnabled()) {
        LOG_WARNING(Render_Vulkan,
                    "TRACE_DRAW strict_compat v99 using forced-null texture path before shader/pipeline setup vertex_batch_size={} enabled_textures={} textures_disabled={}",
...
bool RasterizerVulkan::AccelerateDisplay(const Pica::FramebufferConfig& config,
    ...
    if (IsDrawTraceEnabled()) {
        LOG_INFO(Render_Vulkan,
                 "TRACE_DRAW accelerate_display v99 addr=0x{:08x} width={} height={} stride={} pixel_format={} src_rect=({}, {}, {}, {}) base_valid={} copy_valid={} chosen={} strict_compat={} forced_base_present_view={}",
