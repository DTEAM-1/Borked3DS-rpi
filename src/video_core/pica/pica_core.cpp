...
[[nodiscard]] bool IsSafePicaHwDrawAllowed() {
    // v98 follows plan de travail 1, with the result from the v97 runtime log:
    // v97 reached pre_call AccelerateDrawBatch with stop_after=7 but did not return
    // through visible TRACE_ACCEL_STAGE/post_call lines. Therefore v98 keeps stop_after=7
    // and isolates the programmable PICA vertex shader one step earlier: config/load_flags only.
...
[[nodiscard]] bool IsSafePicaHwEnterAllowed() {
    // Re-testing TRACE_ACCEL_STAGE stage=7 with BORKED3DS_V3DV_PROBE_PROGRAMMABLE_VS_CONFIG_ONLY=1 is the v98 diagnostic step, still not a real draw
...
void PicaCore::DrawArrays(bool is_indexed) {
    ...
    // v98: do not copy GVX64's final software-style behavior. Keep broad PICA acceleration
    ...
    if (accelerate_draw && IsStrictCompatEnabled() && !IsPicaAccelAllowed() &&
        ...
        if (trace_draw) {
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat v98 forcing software path before broad PICA acceleration draw_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} topology={} allow_pica_accel=0 safe_hw_candidate={} safe_hw_allowed={} safe_hw_enter={} safe_hw_dry_run={} safe_hw_index={} safe_hw_budget={}",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
            ...
            LogPicaTextureState(regs.internal, "v98_force_software_before_broad_accel");
        }
        if (!g_logged_strict_accel_gate.exchange(true)) {
            LOG_WARNING(HW_GPU,
                        "Pi5/V3DV strict compatibility v98: broad PICA AccelerateDrawBatch remains disabled by default; tiny untextured HW candidates may enter AccelerateDrawBatch only with the v98 staged preflight enabled. Keep BORKED3DS_V3DV_ACCEL_STAGE_STOP_AFTER=7, BORKED3DS_V3DV_USE_TRIVIAL_VERTEX_SHADER_PROBE=0, BORKED3DS_V3DV_PROBE_PROGRAMMABLE_VS_GENERATE_ONLY=0, and BORKED3DS_V3DV_PROBE_PROGRAMMABLE_VS_CONFIG_ONLY=1 for normal v98 tests");
        }
        accelerate_draw = false;
    } else if (strict_safe_pica_hw_draw && trace_draw) {
        LOG_WARNING(HW_GPU,
                    "TRACE_DRAW_PICA strict_compat v98 safe micro PICA HW candidate hw_index={} budget={} draw_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} topology={} use_hw_shader={} enter={} dry_run={} color_addr=0x{:08x} depth_addr=0x{:08x}",
        ...
        if (IsVerbosePicaMicroTextureTraceEnabled()) {
            LogPicaTextureState(regs.internal, "v98_safe_micro_hw_candidate");
        }
    }
    ...
    if (strict_safe_pica_hw_dry_run) {
        if (trace_draw) {
            LOG_WARNING(HW_GPU,
                        "TRACE_DRAW_PICA strict_compat v98 handoff dry-run consumed safe micro PICA HW candidate before AccelerateDrawBatch draw_index={} indexed={} num_vertices={} hw_index={} budget={} color_addr=0x{:08x} depth_addr=0x{:08x}",
                        draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                        strict_safe_pica_hw_index, GetSafePicaHwDrawBudget(),
                        regs.internal.framebuffer.framebuffer.GetColorBufferPhysicalAddress(),
                        regs.internal.framebuffer.framebuffer.GetDepthBufferPhysicalAddress());
            if (IsVerbosePicaMicroTextureTraceEnabled()) {
                LogPicaTextureState(regs.internal, "v98_safe_micro_hw_dry_run_consumed");
            }
        }
        return;
    }

    // v98 last-chance guard: only explicit full diagnosis or explicitly executed tiny HW
    // probes may enter AccelerateDrawBatch in strict mode.
    if (accelerate_draw && IsStrictCompatEnabled() && !IsPicaAccelAllowed() &&
        !strict_safe_pica_hw_enter) {
        if (trace_draw) {
            LOG_WARNING(HW_GPU,
                        "TRACE_DRAW_PICA strict_compat v98 late guard blocked AccelerateDrawBatch draw_index={} indexed={} num_vertices={} textures_disabled={} topology={} safe_hw_candidate={} safe_hw_allowed={} safe_hw_enter={} safe_hw_dry_run={}",
                        draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                        textures_disabled, static_cast<u32>(primitive_assembler.GetTopology()),
                        static_cast<u32>(strict_safe_pica_hw_candidate),
                        static_cast<u32>(IsSafePicaHwDrawAllowed()),
                        static_cast<u32>(strict_safe_pica_hw_enter),
                        static_cast<u32>(strict_safe_pica_hw_dry_run));
            LogPicaTextureState(regs.internal, "v98_late_guard_force_software");
        }
        accelerate_draw = false;
    }
    ...
    if (accelerate_draw) {
        if (trace_draw) {
            LOG_WARNING(HW_GPU,
                        "TRACE_DRAW_PICA v98 pre_call AccelerateDrawBatch indexed={} draw_index={} num_vertices={} hw_index={} stop_after={} color_addr=0x{:08x} depth_addr=0x{:08x}",
        ...
        if (trace_draw) {
            LOG_WARNING(HW_GPU,
                     "TRACE_DRAW_PICA v98 post_call AccelerateDrawBatch returned {} indexed={} draw_index={} num_vertices={} hw_index={}",
