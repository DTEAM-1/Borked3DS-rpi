...
void PicaCore::DrawArrays(bool is_indexed) {
    ...
    if (is_startup_textured_draw) {
        ...
        const bool keep_startup_textured_software_fallback =
            fragile_startup_draws_seen < 192 || startup_textured_index <= 160;
        ...
        if (keep_startup_textured_software_fallback) {
            if (regs.internal.pipeline.num_vertices >= 12) {
                ...
                if (large_textured_startup_skip_index <= 160) {
                    if (trace_draw) {
                        LOG_INFO(HW_GPU,
                                 "TRACE_DRAW_PICA strict_compat skipping medium_or_large textured startup draw entirely v11 draw_index={} startup_textured_index={} medium_or_large_startup_skip_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} fragile_startup_draws_seen={} startup_textured_window=160 fragile_cooldown_threshold=192 medium_or_large_startup_skip_window=160",
                        ...
                        LogPicaTextureState(regs.internal, "medium_or_large_textured_startup_skip_v11");
                    }
                    return;
                }
            }
            ...
            if (trace_draw) {
                LOG_INFO(HW_GPU,
                         "TRACE_DRAW_PICA strict_compat forcing software fallback for startup textured draw draw_index={} startup_textured_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} fragile_startup_draws_seen={} startup_textured_window=160 fragile_cooldown_threshold=192",
                         draw_index, startup_textured_index, is_indexed,
                         regs.internal.pipeline.num_vertices, primitive_assembler.IsEmpty(),
                         textures_disabled, fragile_startup_draws_seen);
                LogPicaTextureState(regs.internal, "startup_textured_fallback");
            }
            accelerate_draw = false;
        } else if (trace_draw && startup_textured_index <= 16 &&
                   !g_logged_large_textured_startup_bypass.exchange(true)) {
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat allowing startup textured draw acceleration only after extreme extended cooldown draw_index={} startup_textured_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} fragile_startup_draws_seen={} startup_textured_gate=post_extra_extended_cooldown_v11",
                     draw_index, startup_textured_index, is_indexed,
                     regs.internal.pipeline.num_vertices, primitive_assembler.IsEmpty(),
                     textures_disabled, fragile_startup_draws_seen);
            LogPicaTextureState(regs.internal, "startup_textured_accel");
        }
    }
    ...
    if (tiny_textured_startup_candidate && !accelerate_draw &&
        Settings::values.use_hw_shader.GetValue() && fragile_startup_draws_seen >= 128) {
        if (trace_draw) {
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat allowing tiny textured startup draw acceleration only after extended cooldown draw_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} fragile_startup_draws_seen={} tiny_textured_startup_cooldown=192",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                     primitive_assembler.IsEmpty(), textures_disabled,
                     fragile_startup_draws_seen);
            LogPicaTextureState(regs.internal, "tiny_textured_startup_force_post_cooldown");
        }
        accelerate_draw = true;
    } else if (tiny_textured_startup_candidate && !accelerate_draw &&
               fragile_startup_draws_seen < 192) {
        const u64 tiny_textured_startup_skip_index = ++g_tiny_textured_startup_skip_counter;
        if (tiny_textured_startup_skip_index <= 128) {
            if (trace_draw) {
                LOG_INFO(HW_GPU,
                         "TRACE_DRAW_PICA strict_compat skipping tiny textured startup draw entirely draw_index={} tiny_textured_startup_skip_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} fragile_startup_draws_seen={} tiny_textured_startup_skip_window=128 tiny_textured_startup_cooldown=192",
                         draw_index, tiny_textured_startup_skip_index, is_indexed,
                         regs.internal.pipeline.num_vertices, primitive_assembler.IsEmpty(),
                         textures_disabled, fragile_startup_draws_seen);
                LogPicaTextureState(regs.internal, "tiny_textured_startup_skip");
            }
            return;
        }
        ...
        if (trace_draw) {
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat keeping tiny textured startup draw in software fallback draw_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} fragile_startup_draws_seen={} tiny_textured_startup_cooldown=192",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                     primitive_assembler.IsEmpty(), textures_disabled,
                     fragile_startup_draws_seen);
            LogPicaTextureState(regs.internal, "tiny_textured_startup_fallback");
        }
    }
