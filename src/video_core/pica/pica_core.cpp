...
void PicaCore::DrawArrays(bool is_indexed) {
    ...
    if (highdraw_indexed18_textured_precandidate) {
        ...
        if (highdraw_indexed18_skip_index <= highdraw_indexed18_skip_window) {
            if (trace_draw) {
                ...
                LogPicaTextureState(regs.internal, "indexed18_textured_highdraw_skip_v35e");
            }
            return;
        }
        ...
        if (trace_draw && highdraw_indexed18_skip_index == highdraw_indexed18_skip_window + 1) {
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat allowing indexed18 textured high-draw followup after v35e skip window draw_index={} indexed={} num_vertices={} topology={} textures_disabled={} color_addr=0x{:08X} depth_addr=0x{:08X}",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                     static_cast<u32>(primitive_assembler.GetTopology()), textures_disabled,
                     regs.internal.framebuffer.framebuffer.GetColorBufferPhysicalAddress(),
                     regs.internal.framebuffer.framebuffer.GetDepthBufferPhysicalAddress());
            LogPicaTextureState(regs.internal, "indexed18_textured_highdraw_allow_v35e");
        }
    }
    ...
    if (late_startup_textured_pair_precandidate) {
        ...
        if (late_textured_pair_skip_index <= late_textured_pair_skip_window) {
            if (trace_draw) {
                ...
                LogPicaTextureState(regs.internal, "late_startup_textured_pair_skip_v27z");
            }
            return;
        }
        ...
        if (trace_draw &&
            ...
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat allowing late-startup textured pair draw after v27z skip window draw_index={} indexed={} num_vertices={} topology={} textures_disabled={} color_addr=0x{:08X} depth_addr=0x{:08X}",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                     static_cast<u32>(primitive_assembler.GetTopology()), textures_disabled,
                     regs.internal.framebuffer.framebuffer.GetColorBufferPhysicalAddress(),
                     regs.internal.framebuffer.framebuffer.GetDepthBufferPhysicalAddress());
            LogPicaTextureState(regs.internal, "late_startup_textured_pair_allow_v27z");
        }
    }
    ...
    if (early_followup_indexed18_textured_accel_candidate) {
        ...
        if (indexed18_textured_skip_index <= indexed18_textured_skip_window) {
            if (trace_draw) {
                LOG_INFO(HW_GPU,
                         "TRACE_DRAW_PICA strict_compat skipping indexed18 textured followup draw entirely v35e draw_index={} indexed18_textured_skip_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} topology={} color_addr=0x{:08X} depth_addr=0x{:08X} skip_window={} prior_indexed24_skips={} prior_generic_indexed6_skips={} prior_nonindexed96_skips={} prior_nonindexed36_skips={} prior_batch42_skips={} prior_indexed6_textured_skips={} prior_indexed6_untextured_skips={}",
                ...
                LogPicaTextureState(regs.internal,
                                    "indexed18_textured_followup_skip_v35e");
            }
            return;
        } else if (trace_draw &&
            ...
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat allowing indexed18 textured followup draw after v35e skip window draw_index={} indexed={} num_vertices={} topology={} textures_disabled={} color_addr=0x{:08X} depth_addr=0x{:08X}",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                     early_followup_topology_u32, textures_disabled,
                     regs.internal.framebuffer.framebuffer.GetColorBufferPhysicalAddress(),
                     regs.internal.framebuffer.framebuffer.GetDepthBufferPhysicalAddress());
            LogPicaTextureState(regs.internal,
                                "indexed18_textured_followup_allow_v35e");
        }
    }
    ...
    if (early_followup_indexed12_untextured_accel_candidate) {
        ...
        if (indexed12_untextured_skip_index <= indexed12_untextured_skip_window) {
            if (trace_draw) {
                LOG_INFO(HW_GPU,
                         ...
                         g_indexed6_untextured_late_startup_skip_counter.load());
                LogPicaTextureState(regs.internal,
                                    "indexed12_untextured_post_pair_skip_v36e");
            }
            return;
        }
        ...
        if (trace_draw &&
            ...
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat allowing indexed12 untextured post-pair draw after v36e skip window draw_index={} indexed={} num_vertices={} topology={} textures_disabled={} color_addr=0x{:08X} depth_addr=0x{:08X}",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                     static_cast<u32>(primitive_assembler.GetTopology()),
                     textures_disabled,
                     regs.internal.framebuffer.framebuffer.GetColorBufferPhysicalAddress(),
                     regs.internal.framebuffer.framebuffer.GetDepthBufferPhysicalAddress());
            LogPicaTextureState(regs.internal,
                                "indexed12_untextured_post_pair_allow_v36e");
        }
    }
    ...
    if (is_startup_textured_draw) {
        ...
        if (keep_startup_textured_software_fallback) {
            ...
            if (trace_draw && !allow_generic_medium_or_large_startup_skip &&
                regs.internal.pipeline.num_vertices == 18) {
                LOG_INFO(HW_GPU,
                         "TRACE_DRAW_PICA strict_compat bypassing generic medium_or_large startup skip for indexed18 followup v35e draw_index={} startup_textured_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} fragile_startup_draws_seen={} startup_textured_window={} fragile_cooldown_threshold={} use_hw_shader_setting={}",
                         ...
                         static_cast<u32>(use_hw_shader_setting));
                LogPicaTextureState(regs.internal,
                                    "indexed18_textured_followup_bypass_generic_v35e");
            }
        ...
        if (indexed24_untextured_skip_index <= indexed24_untextured_skip_window) {
            if (trace_draw) {
                LOG_INFO(HW_GPU,
                         ...
                         g_indexed6_untextured_late_startup_skip_counter.load());
                LogPicaTextureState(regs.internal,
                                    "indexed24_untextured_first_nonfragile_skip_v34e");
            }
            return;
        } else if (trace_draw &&
            ...
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat allowing first non-fragile indexed24 untextured draw after v34e skip window draw_index={} indexed={} num_vertices={} topology={} textures_disabled={} color_addr=0x{:08X} depth_addr=0x{:08X}",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                     early_followup_topology_u32, textures_disabled,
                     regs.internal.framebuffer.framebuffer.GetColorBufferPhysicalAddress(),
                     regs.internal.framebuffer.framebuffer.GetDepthBufferPhysicalAddress());
            LogPicaTextureState(regs.internal,
                                "indexed24_untextured_first_nonfragile_allow_v34e");
        }
    }


    // indexed18/indexed12 post-index24 followup guards moved earlier in strict-compat order (v35e/v36e).
