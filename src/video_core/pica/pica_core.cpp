...
void PicaCore::DrawArrays(bool is_indexed) {
    ...
    // v60: Pi 5/V3DV hard gate for the PICA accelerated batch path.
    ...
    if (accelerate_draw && IsStrictCompatEnabled() && !IsPicaAccelAllowed()) {
        if (trace_draw) {
            LOG_INFO(HW_GPU,
                     "TRACE_DRAW_PICA strict_compat v60 forcing software path before any PICA acceleration draw_index={} indexed={} num_vertices={} primitive_empty={} textures_disabled={} topology={} allow_pica_accel=0",
                     draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                     primitive_assembler.IsEmpty(), textures_disabled,
                     static_cast<u32>(primitive_assembler.GetTopology()));
            LogPicaTextureState(regs.internal, "v60_force_software_before_accel");
        }
        if (!g_logged_strict_accel_gate.exchange(true)) {
            LOG_WARNING(HW_GPU,
                        "Pi5/V3DV strict compatibility v60: PICA AccelerateDrawBatch hard-disabled by default; set BORKED3DS_V3DV_ALLOW_PICA_ACCEL=1 only for diagnosis");
        }
    ...
    // v60 last-chance guard: never allow the strict Pi5/V3DV path to slip into
    ...
    if (accelerate_draw && IsStrictCompatEnabled() && !IsPicaAccelAllowed()) {
        if (trace_draw) {
            LOG_WARNING(HW_GPU,
                        "TRACE_DRAW_PICA strict_compat v60 late guard blocked AccelerateDrawBatch draw_index={} indexed={} num_vertices={} textures_disabled={} topology={}",
                        draw_index, is_indexed, regs.internal.pipeline.num_vertices,
                        textures_disabled, static_cast<u32>(primitive_assembler.GetTopology()));
            LogPicaTextureState(regs.internal, "v60_late_guard_force_software");
        }
        accelerate_draw = false;
    }
