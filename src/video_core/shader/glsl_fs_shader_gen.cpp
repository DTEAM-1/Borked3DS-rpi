...
vec4 secondary_fragment_color = vec4(0.0);
    ...
    if (config.framebuffer.shadow_rendering) {
        ...
        // v131 identification sweep: BORKED3DS_FS_FORCE_ATF=<n> forces ONLY draws whose
        // alpha_test_func == n to opaque white, leaving the rest of the scene as context. v130
        // proved atf==6 lights the PLANTS, not the text, so the "text uses atf6" note in the older
        // comments is wrong -- the working AND broken text use some other compare-func value. This
        // turns identification into a cheap no-rebuild sweep: set n = 0..7 across relaunches (cache
        // purged each time so shaders regenerate and re-read the env) and note which value whitens
        // (a) the working small-box text, (b) the broken large-box text and the white marker
        // rectangle in the lower-right of the failing zones. That value is the broken-text draw's
        // signature; once known, every future text probe can gate on it precisely instead of
        // guessing. alpha_test_func is a 3-bit field, so the full candidate set is 0..7
        // (Never/Always/Equal/NotEqual/Less/LessEqual/Greater/GreaterEqual in PICA order).
        {
            const char* fatf_env = std::getenv("BORKED3DS_FS_FORCE_ATF");
            if (fatf_env != nullptr && fatf_env[0] != '\0') {
                const int want_atf = std::atoi(fatf_env);
                if (want_atf >= 0 &&
                    static_cast<int>(config.framebuffer.alpha_test_func.Value()) == want_atf) {
        ...
        // v115-E debug probe: BORKED3DS_FS_SHOW_PRIMARY_ALPHA=1 forces every draw to an opaque
        // grayscale field whose luminance encodes ONLY primary_color.a (the vertex/per-vertex
        // color alpha that the VS produced from vertex input attribute reg1). NON-CHROMATIC
        // (luminance only, colorblind-safe). This isolates the vertex-color alpha from the
        // texture alpha, to settle whether the text discard is caused by the vertex side.
        // The text TEV computes final alpha = primary_color.a * tex0.a (confirmed from the
        // dumped atf6 fragment shaders), so if primary_color.a is 0 the whole product is 0 and
        // every glyph fragment is discarded by the alpha test even though the atlas (tex0.a) is
        // correct. Reading: a WHITE/BRIGHT dialog box => primary_color.a == 1 (vertex alpha is
        // fine, defect is downstream in tex0.a sampling); a BLACK/dark box => primary_color.a == 0
        // => the vertex color alpha is being lost. On Vulkan this happens when a 3-component
        // (RGB) vertex color attribute is fed to the pipeline and its missing W/alpha component
        // defaults to 0 instead of 1 (the classic GL-vs-Vulkan default-W difference), which would
        ...
        // vBALL-FACET sondes de normale. Teste directement l'ombrage plat par facette : la normale
        // interpolee 'normal' (apres rotation par le quaternion) est ce qui pilote l'eclairage.
        // Non chromatique (daltonien). Necessite l'eclairage actif ('normal' n'existe que la).
        // SHOW_NORMAL_Z : luminance = normal.z remappe [0,1]. Sur une sphere a normales LISSES ->
        // degrade radial doux (clair au centre, sombre aux bords). Sur des normales PLATES (de
        // face) -> facettes a teinte plate avec aretes nettes. C'est le test decisif du facettage.
        if (config.lighting.enable) {
            const char* p = std::getenv("BORKED3DS_FS_SHOW_NORMAL_Z");
            if (p != nullptr && p[0] == '1') {
                out += "{ float _l = clamp(normal.z * 0.5 + 0.5, 0.0, 1.0);\n"
                       "  color = vec4(_l, _l, _l, 1.0); }\n";
            }
        }
        // SHOW_NORMAL_EDGES : luminance = amplitude de la derivee ecran de la normale
        // (length(fwidth(normal))). ~0 a l'interieur d'une surface lisse, PIC aux frontieres. Si
        // les normales sont plates par triangle, ce probe dessine le FILAIRE des triangles (aretes
        // claires sur fond noir) -> facettage incontestable. Si tout reste noir -> normales lisses.
        if (config.lighting.enable) {
            const char* p = std::getenv("BORKED3DS_FS_SHOW_NORMAL_EDGES");
            if (p != nullptr && p[0] == '1') {
                out += "{ float _e = clamp(length(fwidth(normal)) * 6.0, 0.0, 1.0);\n"
                       "  color = vec4(_e, _e, _e, 1.0); }\n";
            }
        }
    }
...
void FragmentModule::DefineExtensions() {
    ...
    if (config.framebuffer.shadow_rendering) {
        // Pi 5 / V3DV Vulkan compatibility:
        // keep the Vulkan GLSL shadow path simple and avoid interlock/shadow export features
        // that have led to SPV_EXT_shader_stencil_export being generated by glslang.
        if (profile.is_vulkan) {
            use_fragment_shader_interlock = false;
        } else {
            use_fragment_shader_interlock = true;
        }
        if (use_fragment_shader_interlock && profile.has_fragment_shader_interlock) {
            out += "#ifdef GL_ES\n";
